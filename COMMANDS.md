...
| rotate | `!rotate <degrees>` | `-inf to inf` | Rotate the video by `<degrees>`. Default value is 45 degrees.<br>FFMPEG expressions not supported, see `!rotaterad`. | `!rotate`<br><br>`!rotate 180` |
| rotaterad | `!rotaterad <radians>` | `-inf to inf` | Rotate the video by `<radians>`. Default value is PI/4 radians.<br>You can use FFMPEG expressions to create an animated rotation, for example: `!rotaterad 2 * PI * sin(t)`. | `!rotaterad`<br><br>`!rotaterad PI/2`<br><br>`!rotaterad t`<br><br>`!rotaterad PI/8 * sin(t)` |
